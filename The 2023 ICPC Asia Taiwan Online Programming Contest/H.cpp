...
void solve(){
    ...
    int limit = __lg(n);
    pw[0] = 1;
    for(int i = 1; i <= limit + 1; i++) pw[i] = pw[i - 1] * 2;
    ll Res = n - pw[limit] + 1;

    ll Ans = 0;
    for(int i = 0; i <= limit; i++){
        /// Nhánh đầy đủ
        ll Sum = min(n - pw[i] + 1, pw[i]);
        
        if(k - 1 < i) break;
        if(n - k + 1 >= pw[limit - i + 1] - 1)
            Ans += Res / pw[limit - i];
        if(Res % pw[limit - i] != 0 && n - k + 1 >= pw[limit - i] - 1 + Res % pw[limit - i])
            Ans++;
        if(n - k + 1 >= pw[limit - i] - 1)
            Ans += Sum - Res / pw[limit - i] - (Res % pw[limit - i] > 0);
